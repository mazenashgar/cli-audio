...
class FrontEnd:
    ...
    def menu(self, args): #method for creating menu
        ...
        while True:
            ...
            if height >= 16 and width >= 60:
                ...
                self.stdscr.addstr(0,0, "cli-audio",curses.A_REVERSE) #paint the string at (0,0) a reverse video status line is displayed
    ...
    def updateSong(self):
        ...
        self.stdscr.addstr(15,10, "Now playing: " + self.player.getCurrentSong()) #if a song is updated the current song is displayed

    def changeSong(self):
        
         changeWindow = curses.newwin(5, 40, 5, 50) #return a new window
             changeWindow.border() #draws a border around the new window
             changeWindow.addstr(0,0, "What is the file path?", curses.A_REVERSE) #displays the specified string on the new wiindow
             self.stdscr.refresh() #updates the window
    ...
    def displayDir(self):
        ...
        libraryWindow.addstr(0,0, "Available songs:", curses.A_REVERSE) #paint the string at (0,0) a reverse video status line is displayed
        placement = 1
        for element in range(len(self.mediaFiles)):
            if self.mediaFiles[element] != 'README.md' and self.mediaFiles[element] != '.DS_Store': #valid media file
                 libraryWindow.addstr(placement, 1, self.mediaFiles[element][:-4]) #prints the specified string at the given position on window
                 self.stdscr.refresh() #updates the window
